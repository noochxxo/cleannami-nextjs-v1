...
// The component now accepts the property and its active subscription
interface LeftColumnProps {
    property: PropertyDetails;
    subscription: PropertyDetails['activeSubscription'];
}

export default function PropertyDetailsLeftColumn({ property, subscription }: LeftColumnProps) {
  return (
      ...
      <Card icon={<HouseHeart className="h-6 w-6" />} title="Property Vitals">
        <div className="grid grid-cols-3 gap-4 text-center">
          <div>
            ...
            <p className="text-xl font-bold text-gray-800">{property.bedCount}</p>
          </div>
          <div>
            <p className="text-sm text-gray-500">Baths</p>
            <p className="text-xl font-bold text-gray-800">{property.bathCount}</p>
          </div>
          <div>
      ...
      <Card icon={<CreditCardIcon />} title="Subscription">
        {subscription ? (
            <>
                ...
                <p className="text-sm">
                Term: <span className="font-semibold">{subscription.durationMonths} Months</span>
                </p>
                <p className="text-sm">
                Renews:{" "}
                <span className="font-semibold">{subscription.startDate ? new Date(subscription.startDate).toLocaleDateString() : 'N/A'}</span>
                </p>
            ...
            <p className="text-sm text-gray-500">No active subscription for this property.</p>
        )}
      </Card>
    </div>
  );
